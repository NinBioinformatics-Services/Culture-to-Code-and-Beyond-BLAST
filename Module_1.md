...
Neighbor-Joining is a commonly used distance-based tree-building method.

Basic Concept

The method starts with a distance matrix and progressively joins sequences or groups of sequences that are considered appropriate neighbors.
...
Limitation:
It relies on the distance information and does not evaluate the complete sequence likelihood in the way Maximum Likelihood does.
```
...
```
Maximum Likelihood is a model-based approach to phylogenetic inference.

It evaluates different possible trees and estimates how likely the observed sequence data are under a particular evolutionary model.

Simplified Workflow
Aligned Sequences
       ↓
Select Evolutionary Model
       ↓
Evaluate Candidate Trees
       ↓
Calculate Likelihood
       ↓
Select Tree with Better Likelihood
Important Concept
...
```
How Does Phylogenetic Bootstrapping Work?

Simplified process:

Original Alignment
       ↓
Randomly Resample Alignment Columns
       ↓
...
Suppose a branch appears in:

950 out of 1000 replicates
